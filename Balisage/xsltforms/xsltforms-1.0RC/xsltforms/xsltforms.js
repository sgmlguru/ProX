	...
	XsltForms_browser.saveXML = function(node, relevant, indent) {
		if (node.nodeType === XsltForms_nodeType.ATTRIBUTE) { 
			...
			if (node.nodeType === XsltForms_nodeType.TEXT) {
				...
				var resultDocument = indent ? XsltForms_browser.xsltProcessorIndent.transformToDocument(node) : relevant ? XsltForms_browser.xsltProcessorRelevant.transformToDocument(node) : XsltForms_browser.xsltProcessor.transformToDocument(node);
