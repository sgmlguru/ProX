  ...
  <xsl:template match="body">
    <xsl:choose>
      <xsl:when test="$demo = 'cargotec'">
        <!-- This template is found in demo.xsl -->
        <xsl:call-template name="body-cargotec"/>
      </xsl:when>
      <xsl:otherwise>
    ...
    <fo:page-sequence master-reference="contents">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$cover.page='1' and not(//back)">
          <xsl:attribute name="initial-page-number">1</xsl:attribute>
          <xsl:attribute name="force-page-count">odd</xsl:attribute><!-- (back cover follows, change for index handling) -->
        </xsl:when>
        <xsl:otherwise>
