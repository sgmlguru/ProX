    ...
    <xsl:if test="section">
      ...
      <fo:page-sequence master-reference="back">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$cover.page='1' and not(glossary) and not(reference) and $legal.page='0'">
      ...
      <fo:page-sequence master-reference="back">
        ...
        <fo:flow flow-name="xsl-region-body">
          <fo:block break-before="page" space-after="8pt" start-indent="-10mm"
            font-family="sans-serif" font-size="{$heading1-font-size}" font-weight="bold"
            id="glossary">
      ...
      <fo:page-sequence master-reference="pub-info">
        <fo:flow flow-name="xsl-region-body">
          <fo:block font-family="sans-serif" font-weight="bold" font-size="12pt"
            ...
            <xsl:text>  </xsl:text>
            <xsl:call-template name="getString">
              <xsl:with-param name="stringName" select="'footer-text'"/><!-- Company name -->
            </xsl:call-template>
          </fo:block>

          <fo:block font-family="serif" font-weight="normal" color="black" text-align="start"
            text-indent="0.0cm" padding-left="5mm" space-after="12pt">
