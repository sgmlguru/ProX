    ...
    <fo:table start-indent="0mm" table-layout="fixed">
      ...
      <fo:table-body>
        ...
        <xsl:if test="$toc.depth=1">
          ...
          <xsl:choose>
            <xsl:when test="$demo = 'cargotec'">
              <!-- in demo.xsl -->
              <xsl:call-template name="one-level-toc-cargotec"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:apply-templates
        ...
        <xsl:if test="$toc.depth=2">
          ...
          <xsl:choose>
            <xsl:when test="$demo = 'cargotec'">
              <!-- in demo.xsl -->
              <xsl:call-template name="two-level-toc-cargotec"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:apply-templates
                select="//section[parent::body]/title |
        ...
        <xsl:if test="$toc.depth=3">
          <xsl:choose>
            <xsl:when test="$demo = 'cargotec'">
              <!-- in demo.xsl -->
              <xsl:call-template name="three-level-toc-cargotec"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:apply-templates
                select="//section[parent::body]/title |
        ...
        </xsl:if><!-- REMOVE comment markers for index in TOC -->
  ...
  <xsl:template name="toc-numbering-choice">
    <xsl:choose>
      <xsl:when test="ancestor::body">
        <xsl:number
          level="multiple"
          count="section[ancestor::body]|inset[ancestor::body]"
          format="1.1"/>
      </xsl:when>
