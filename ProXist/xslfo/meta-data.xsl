    ...
    <xsl:if test="$cover.page != '1'">
      ...
      <fo:page-sequence master-reference="meta-data" initial-page-number="auto-odd"
        ...
        <fo:flow flow-name="xsl-region-body">
          ...
          <fo:block hyphenate="false" font-family="sans-serif" font-size="24pt" font-weight="bold"
            border-after-style="double" start-indent="-10mm" space-after="6pt">
            ...
            <xsl:choose>
              ...
              <xsl:when test="//meta-data/doc-info/doc-type/@type = 'order-conf'">
                <fo:block margin-bottom="6pt">
                  <xsl:call-template name="getString">
                    <xsl:with-param name="stringName" select="'Order Confirmation'"/>
                  </xsl:call-template>
                </fo:block>
              </xsl:when>
              ...
              <xsl:when test="//meta-data/doc-info/doc-type/@type = 'test-spec'">
                <fo:block margin-bottom="6pt">
                  <xsl:call-template name="getString">
                    <xsl:with-param name="stringName" select="'Test Specification'"/>
                  </xsl:call-template>
                </fo:block>
              </xsl:when>
              <xsl:when test="//meta-data/doc-info/doc-type/@type = 'system-spec'">
                <fo:block margin-bottom="6pt">
                  <xsl:call-template name="getString">
                    <xsl:with-param name="stringName" select="'System Specification'"/>
                  </xsl:call-template>
                </fo:block>
              </xsl:when>
          ...
          <fo:block font-family="sans-serif" font-size="10pt" text-align="start">
            ...
            <xsl:if test="abstract">
              <fo:block font-size="{$heading1-font-size}" font-weight="bold" start-indent="-10mm"
                space-before="24pt" space-after="8pt" id="abstract">
    ...
    <xsl:if test="$cover.page='1'">
      <fo:page-sequence master-reference="cover">
        ...
        <fo:static-content flow-name="xsl-region-after-cover">
          ...
          <xsl:choose>
            <xsl:when test="$demo = 'gunnebo'">
              <xsl:call-template name="gunnebo-cover-logo"/>
            </xsl:when>
            <xsl:otherwise>
              <fo:block font-family="sans-serif" text-align="center" display-align="after">
                <xsl:if test="$first.page.footer.logo=1">
                  ...
                  <xsl:call-template name="logo-ref"/><!-- See static-content.xsl -->
                </xsl:if>
              </fo:block>
            </xsl:otherwise>
          </xsl:choose>
          ...
          <fo:block space-before="0pt" text-align="center" font-size="8pt" font-weight="bold">
...
<!--                  <xsl:value-of select="'Gunnebo'"/>-->
        ...
        <fo:flow flow-name="xsl-region-body">
          ...
          <!-- Document Title -->
          <fo:block font-family="sans-serif" font-size="32pt" font-weight="bold" text-align="end" id="doc-title">
            <xsl:value-of select="$doc-title"/>
          </fo:block>

          <!-- Document Subtitle -->
          <fo:block space-before="5mm" font-family="sans-serif" font-size="14pt" font-weight="bold"
            text-align="end">
            <xsl:value-of select="$subtitle"/>
          </fo:block>
          ...
          <xsl:choose>
            <xsl:when test="$demo = 'gunnebo'">
              <!-- Cover image, Gunnebo -->
              <xsl:call-template name="gunnebo-covers"/>
            </xsl:when>
            <xsl:otherwise>
              <!-- Cover image COSML -->
            </xsl:otherwise>
      ...
      <fo:page-sequence force-page-count="end-on-even" master-reference="meta-data" format="i">
        ...
        <fo:flow flow-name="xsl-region-body">
          <fo:block font-family="sans-serif" font-size="10pt" text-align="start">
            ...
            <xsl:choose>
              <xsl:when test="$demo = 'gunnebo'">
                <xsl:call-template name="gunnebo-pub-info-image"/>
              </xsl:when>
              <xsl:otherwise>
            ...
            <fo:table-cell>
              <fo:block>
                ...
                <xsl:for-each select="//author/name">
                  <xsl:apply-templates select="first"/>&#xA0;<xsl:apply-templates select="last"/>
  ...
  <xsl:template name="back-cover">
    <fo:page-sequence master-reference="back-cover">
      ...
      <fo:static-content flow-name="xsl-region-after-back-cover">
        ...
        <xsl:choose>
          <xsl:when test="$demo = 'gunnebo'">
            <xsl:call-template name="gunnebo-back-cover-logo"/>
          </xsl:when>
          <xsl:otherwise>
            <fo:block font-family="sans-serif" text-align="center" display-align="after">
              <xsl:if test="$first.page.footer.logo=1">
      ...
      <fo:flow flow-name="xsl-region-body">
        <xsl:choose>
          <xsl:when test="$demo = 'gunnebo'">
            <!-- Back cover image, Gunnebo -->
            <xsl:call-template name="gunnebo-back-covers"/>
          </xsl:when>
          <xsl:otherwise>
            <!-- Cover image COSML -->
            <fo:block>
