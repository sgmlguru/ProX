...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="demo">
    <xsl:choose>
      ...
      <xsl:when test="/*/@role = 'fichet'">
        <xsl:value-of select="'fichet'"/>
      </xsl:when>
      <xsl:when test="/*/@role = 'envac'">
        <xsl:value-of select="'envac'"/>
      </xsl:when>
    ...
    <xsl:choose>
      <xsl:when test="$double.sided != 0">
        <xsl:if test="$paper.type='A4'">
          <xsl:choose>
            <xsl:when test="$demo = 'envac'">2mm</xsl:when>
            <xsl:otherwise>30mm</xsl:otherwise>
          </xsl:choose>
        </xsl:if>
        <xsl:if test="$paper.type='A5'">
          <xsl:choose>
            <xsl:when test="$demo = 'envac'">0mm</xsl:when>
  ...
  <xsl:param name="body.margin.outer">
    <xsl:choose>
      <xsl:when test="$double.sided != 0">
        <xsl:if test="$paper.type='A4'">
          <xsl:choose>
            <xsl:when test="$demo = 'envac'">5mm</xsl:when>
        ...
        <xsl:if test="$paper.type='A5'">
          <xsl:choose>
            <xsl:when test="$demo = 'envac'">0mm</xsl:when>
            <xsl:otherwise>12.5mm</xsl:otherwise>
          </xsl:choose>
        </xsl:if>
      </xsl:when>
      <xsl:otherwise>30mm</xsl:otherwise>
  ...
  <xsl:param name="region.before.extent">
    <xsl:choose>
      <!-- Envac demo -->
      <xsl:when test="$demo = 'envac'">
        <xsl:if test="$paper.type='A4'">19mm</xsl:if>
        <xsl:if test="$paper.type='A5'">16mm</xsl:if>
      </xsl:when>
      <!-- COS Standard -->
      <xsl:otherwise>
        <xsl:if test="$paper.type='A4'">25mm</xsl:if>
  ...
  <xsl:param name="region.after.extent">
    <xsl:choose>
      ...
      <xsl:when test="$demo = 'envac'">
        ...
        <xsl:if test="$paper.type='A5'">7mm</xsl:if>
      </xsl:when>
      <!-- COS Standard -->
      <xsl:otherwise>
        <xsl:if test="$paper.type='A4'">25mm</xsl:if>
  ...
  <xsl:param name="body-font-family">
    <xsl:choose>
      ...
      <xsl:otherwise>Times, MS-Gothic, LucidaSans, 'Zapf Dingbats', Symbol, Tahoma</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  ...
  <xsl:param name="body-font-size">
    <xsl:choose>
      <!-- Envac demo -->
      <xsl:when test="$demo = 'envac'">
        <xsl:if test="$paper.type='A4'">9pt</xsl:if>
  ...
  <xsl:param name="heading2-font-size">
    <xsl:choose>
      <!-- Envac Demo -->
      <xsl:when test="$demo = 'envac'">
        <xsl:value-of select="substring-before($body-font-size, 'pt') * 1.0"/>
        <xsl:text>pt</xsl:text>
      </xsl:when>
      <!-- COS Standard -->
      <xsl:otherwise>
  ...
  <xsl:param name="standard.files.dir"
    select="'/Users/System/Documents/SGML/DTD/Cassis/FO/'"/>
